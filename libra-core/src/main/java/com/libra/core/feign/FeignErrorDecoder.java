 ...
 * @author Libra
 * @date 2018/11/28
 * @description 自己的feign错误解码器（为了feign接收到错误的返回，转化成roses可识别的ServiceException）
 */
@Slf4j
public class FeignErrorDecoder implements ErrorDecoder {
    ...
    public Exception decode(String methodKey, Response response) {
         ...
         */
        if (EmptyUtil.isNotEmpty(exceptionClazz)) {
    ...
    private ApiServiceException getApiServiceExceptionByClassName(String className, Integer code, String message) {
        ...
        try {
            ...
            Constructor constructor = clazz.getConstructor(BaseExceptionEnum.class);
            return (ApiServiceException) constructor.newInstance(new BaseExceptionEnum() {
                @Override
